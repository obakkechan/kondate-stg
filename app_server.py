...
def get_html_regist_button():
    ...
    for name in table_name:
        ...
        if num == 3:
            register = register + "</tr><br><tr>"  # ボタン3個で改行する。
            num = 0
...
def regist_food_input():
    ...
    """
    col_show = ["カロリー(kcal)", "炭水化物(g)", "タンパク質(g)", "脂質(g)", "ビタミンA(μgRAE)", "ビタミンB1(mg)", "ビタミンB2(mg)", "ビタミンB6(mg)",
                "ビタミンB12(μg)", "ビタミンC(mg)", "ビタミンE(mg)", "カルシウム(mg)", "鉄分(mg)", "食物繊維(g)", "塩分(g)"]
    col = ["calorie_kcal", "carbohydrate_g", "protein_g", "lipid_g", "vitaminA_μgRAE", "vitaminB1_mg", "vitaminB2_mg",
           "vitaminB6_mg", "vitaminB12_μg", "vitaminC_mg", "vitaminE_mg", "calcium_mg", "fe_mg", "dietary_fiber_g",
           "salt_g"]
    for num in range(15):
        contents = contents + f"""
            <tr>
